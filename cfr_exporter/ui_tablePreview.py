...
from cfr_exporter.table_to_data import table_html_to_df
from cfr_exporter.table_formatter import format_df
#from cfr_exporter.table_parser import format_df, table_html_to_df
from cfr_exporter.workbook_builder import (
    make_source_key,
    sanitize_sheet_name,
    workbook_has_table,
)
    ...
    with add_col1:
        if st.button("Add formatted table to workbook"):
            if workbook_has_table(st.session_state.workbook_tables, formatted_source_key):
                st.warning("That formatted table is already in the workbook.")
            else:
